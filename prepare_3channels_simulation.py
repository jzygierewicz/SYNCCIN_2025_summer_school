...
x0[10] = 3 # This is a marker to visualize the delays between the channels better.

"""Create noisy copies of the signal."""

N = x0.shape[0]
A = 1 # amplitude of noise proportional to the amplitude of signal
x1 = x0 + noise_factor * A*np.random.randn(N)
x2 = x0 + noise_factor * A*np.random.randn(N)
...
sim = {'EEG': sim1, 'Fs': Fs, 'channels': {'O1': 0, 'O1_1dt': 1, 'O1_2dt': 2}}
joblib.dump(sim, 'simulated_3_channels.joblib')
